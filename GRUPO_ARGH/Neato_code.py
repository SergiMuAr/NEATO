...
if __name__ == "__main__":
	...
	for i in range (0,100):
		...
		print var
		print distanceDetection
		print calibration
		centralLeftSensorValue = (var[3]/calibration if var[3] < distanceDetection else 0)
		outerLeftSensorValue = (var[4]/calibration if var[4] < distanceDetection else 0)
		centralRightSensorValue = (var[1]/calibration if var[1] < distanceDetection else 0)
		outerRightSensorValue = (var[0]/calibration if var[0] < distanceDetection else 0)
		centralSensorValue = (var[2]/calibration if var[2] < distanceDetection else 0)
		...
		if abs(left) <= threshold and abs(right) <= threshold:
			...
			if state != ROTATING:
				...
				timesRotated = 0
			state = ROTATING
		state = ROTATING #debug
		if state == IDLE:
			print 'IDLE'
			if timeStepCount < timesRotated:
				rotLeft = leftRotated/timesRotated
				rotRight = rightRotated/timesRotated
				timeStepCount += 1
				#velLeft = initialVelocity + rotLeft
				#velRight = initialVelocity + rotRight
				leftMotor = initialVelocity - rotLeft
				rightMotor = initialVelocity - rotRight
			else:
				leftMotor = 100
				rightMotor = 100
		elif state == ROTATING:
			...
			leftMotor = 50
			rightMotor = 50
			#envia(ser, 'SetMotor LWheelDist '+ str(10) +' RwheelDist ' + str(10) + ' Speed ' + str(velocity_max))
