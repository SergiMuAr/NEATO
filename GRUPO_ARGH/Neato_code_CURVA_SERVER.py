...
def odometry(L, R):
    ...
    return [pos_wordl[0][0], pos_wordl[1][0], pos_wordl[2][0], pos_wordl_est[0][0], pos_wordl_est[1][0], pos_wordl_est[2][0]]
...
if __name__ == "__main__":
	...
	try:
		while i < len(sys.argv):
			...
			L, R = get_motors()
			Odo = odometry(L, R)
			move(X,Y, odometry_queue, laser_queue, laser_set)
			print "Moving finished, final position: " + str(Odo[0]) + ", " + str(Odo[1]) + ". Estimated position: " +  str(Odo[3]) + ", " + str(Odo[4]) 
